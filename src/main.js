...
scene.fog = new THREE.Fog(0x141b26, 12, 32);
...
const fillLight = new THREE.DirectionalLight(0x9ecbff, 0.75);
...
const lampBase = new THREE.Mesh(
  ...
  new THREE.MeshStandardMaterial({ color: 0x1a202b, roughness: 0.4, metalness: 0.3 })
);
lampBase.position.set(1.4, -0.32, -1.05);
lampBase.castShadow = true;
lavaLampGroup.add(lampBase);

const lampGlass = new THREE.Mesh(
  new THREE.CylinderGeometry(0.14, 0.14, 0.5, 24),
...
lampGlass.position.set(1.4, -0.02, -1.05);
lavaLampGroup.add(lampGlass);

const lampGlow = new THREE.Mesh(
  new THREE.CylinderGeometry(0.1, 0.12, 0.42, 24),
  new THREE.MeshStandardMaterial({
    color: 0xff4fd8,
    emissive: 0xff4fd8,
...
lampGlow.position.set(1.4, -0.02, -1.05);
lavaLampGroup.add(lampGlow);

const lampLight = new THREE.PointLight(0xff4fd8, 0.6, 3);
lampLight.position.set(1.4, 0.2, -1.05);
lavaLampGroup.add(lampLight);
...
toggleButton.addEventListener("click", () => {
  isDay = !isDay;
  ambientLight.intensity = isDay ? 0.95 : 0.4;
  keyLight.intensity = isDay ? 1.5 : 0.7;
  fillLight.intensity = isDay ? 0.75 : 0.35;
  rimLight.intensity = isDay ? 0.35 : 0.15;
  roomMaterial.color.set(isDay ? 0x1b2433 : 0x0d1218);
  scene.background.set(isDay ? 0x141b26 : 0x06080c);
});
